...
if len(sys.argv) != 3:
    print(f'Usage: {sys.argv[0]} <lib_name> <module_name>')
...
cmake_root_header = '''# Copyright (c) 2019-2021 The STE||AR-Group
#
# SPDX-License-Identifier: BSL-1.0
# Distributed under the Boost Software License, Version 1.0. (See accompanying
# file LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
...
cmake_header = '''# Copyright (c) 2020-2021 The STE||AR-Group
#
# SPDX-License-Identifier: BSL-1.0
# Distributed under the Boost Software License, Version 1.0. (See accompanying
# file LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
...
root_cmakelists_template = f'''{cmake_root_header}
...
modules_cmakelists = f'''{cmake_header}
...
modules_cmakelists += f'''
hpx_info("")
hpx_info("  Configuring libhpx{f"_{lib_name}" if lib_name != "full" else ""} modules:")
...
header_name_str = (
    "Main |hpx| modules"
    if lib_name == "full"
    else f"{lib_name.capitalize()} modules"
)
